...
const VerifyOtp = () => {
  ...
  const {firstName,lastName,uid,password,confirmPassword,email,cType} = userform;
      ...
      try{
        ...
        const res = await apiConnecter("POST","https://e-ballot-server.vercel.app/api/v1/auth/signup",{firstName,lastName,uid,password,confirmPassword,email,otp,contactNumber,cType});
        console.log(res);
        if (ref.current) {
